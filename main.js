...
document.getElementById("LoadFile").onclick = () => {
    ...
    if (fileInput.files.length > 0) {
        ...
        reader.onload = function (e) {
            ...
            for (let i in jsonContent.data) {
                tileManager.AddTile(new Tile(jsonContent.data[i].time));
            }
        };
...
document.getElementById("Load").onclick = () => {
    ...
    if (audioInput.files.length > 0) {
        ...
        reader.onload = function (event) {
            ...
            tileManager = new TileManager(wavesurfer.getDuration());
            loaded = true;
        };
...
function DrawTile() {
    ...
    for (let i = 0; i < tileManager.tiles.length; i++) {
        ...
        const x =
            (tile.time / wavesurfer.getDuration()) * canvas.width -
            tileSize[0] / 2;
        tile.x = x;
